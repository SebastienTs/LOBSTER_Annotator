...
switch tag
    ...
    case 'Left Click'
        ...
        if brush.windowing
           ...
           slice = mat2gray(slice,[double(L-W/2) double(L+W/2)]);
        else
        ...
        %use BW mask that has the largest number of elements
        %if sum(BW(BW)) < sum(mask(mask))/2
        %    BW= ~BW & mask;
        %end
        
        %use BW mask that has the smallest number of elements
        %if sum(BW(BW)) >= sum(mask(mask))/2
        %    BW= ~BW & mask;
        %end
        ...
        %mask = BW | maskOld;
        maskOld(BW>0) = 0;  
        mask = uint8(single(LastColor)*BW) + maskOld;
