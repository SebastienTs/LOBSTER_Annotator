%%% Note: Modified version, some critical properties are set public

classdef imtool3DLbl_mod < handle
    ...
    methods
        ...
        function tool = imtool3DLbl_mod(varargin)  %Constructor
            ...
            tool.handles.Info=uicontrol(tool.handles.Panels.Info,'Style','text','String','(x,y) val valmsk','Units','Normalized','Position',[0 .1 .5 .8],'BackgroundColor','k','ForegroundColor','w','FontSize',12,'HorizontalAlignment','Left');
            ...
            icon_profile = makeToolbarIconFromPNG([iptdir '/draw_assistedFreehand_24.png']);
        ...
        function setMaskColor(tool,maskColor)
            ...
            C = zeros(size(tool.I,1),size(tool.I,2),3);
            C(:,:,1)=double(intlut(uint8(mask(:,:,1)), tool.col(:,1))/256); 
            C(:,:,2)=double(intlut(uint8(mask(:,:,1)), tool.col(:,2))/256); 
            C(:,:,3)=double(intlut(uint8(mask(:,:,1)), tool.col(:,3))/256);
        ...
        function setImage(varargin)
            ...
            C(:,:,1)=tool.maskColor(1); 
            C(:,:,2)=tool.maskColor(2); 
            C(:,:,3)=tool.maskColor(3);
            set(tool.handles.mask,'CData',C);
    ...
    methods (Access = private)
        ...
        function showSlice(varargin)
            ...
            %% Check if there is a mask to display in this slice (speedup)
            SkipMask = 0;
            if tool.alpha > 0 && RunProjViewer == 0
                if sum(tool.mask(:,:,n)) == 0
                    SkipMask = 1;
                end 
            end
            ...
            if tool.alpha > 0 && SkipMask == 0
                set(tool.handles.I,'AlphaData',1-0.5*(tool.alpha>0)*(tool.mask(:,:,n)>0));
            else
            ...
            if tool.alpha > 0 && SkipMask == 0
                if RunProjViewer > 1
                    CurrMask = 0.5*max(tool.mask(:,:,max(n-floor((RunProjViewer-1)/2),1):min(n+floor((RunProjViewer-1)/2),size(tool.mask,3))),[],3)+0.5*tool.mask(:,:,n);
                else
                    CurrMask = tool.mask(:,:,n);
                end
                %% Modified by Seb to show "randomized" LUT 
                im = single(zeros(size(tool.I,1),size(tool.I,2),3));
                im(:,:,1) = mod(CurrMask,3)/3;
                im(:,:,2) = mod(CurrMask,7)/7; 
                im(:,:,3) = mod(CurrMask,15)/15;
                set(tool.handles.mask,'CData',im); 
                set(tool.handles.mask,'AlphaData',tool.alpha*CurrMask);
            else
                set(tool.handles.mask,'AlphaData',0);
            end
