function render3d_and_mesh(id,V,FV)
    ...
    if(data.FirstRender)
       ...
       Faces = FV.faces';
       Vertices = Vertices';
       data.MeshOpenGLID = Gl.glGenLists(1); 
       Gl.glNewList(data.MeshOpenGLID, Gl.GL_COMPILE);
       ...
       if isfield(FV,'colors')
        Gl.glEnableClientState(Gl.GL_COLOR_ARRAY);
        colors = FV.colors;
        Gl.glColorPointer(3, Gl.GL_UNSIGNED_BYTE, 0, colors );
       end
       if isfield(FV,'width')
        Gl.glLineWidth(FV.width);
       end
       Gl.glVertexPointer(3, Gl.GL_FLOAT, 0, VertexArray);
       if isfield(FV,'points')
        Gl.glEnable(Gl.GL_PROGRAM_POINT_SIZE_EXT);
        Gl.glPointSize(FV.pointsize);   
        Gl.glDrawElements( Gl.GL_POINTS, numel(Faces), Gl.GL_UNSIGNED_INT, FacesArray );
       else
        Normals = FV.Normals;   
    ...
    if data.showmesh == 1
        ...
        Gl.glCallList(data.MeshOpenGLID);
    end
